...
export default function page({params}) {
  ...
  const shuffleArtefact = async (artefacts) => {
    if (artefacts) {
      const shuffled = [...artefacts]; // Create a shallow copy of the artefacts array
  ...
  useEffect(() => {
    shuffleArtefact(artefacts)
  }, [artefacts, artefactData]);

  return (
      <Layouts>
        {
          artefactData?.length === 0 ? <NotFound /> : (
                  ...
                  <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
                        ...
                        >
                          <div className={`w-full pr-3 px-6`}>
                            <div className={`flex gap-5 items-center mb-3`}>
                              <span class="bg-blue-100 text-primary text-sm font-medium px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-blue-400 border border-blue-400">Artefak</span>
                              <h1 className={`text-gray-400 text-sm`}>{artefactData.createdAt}</h1>
                            </div>
                        ...
                        >
                          <div className={`pr-3 px-6`}>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                              {
                                artefactData.galleries?.map(item => (
                                    <div>
                                      <img class="h-auto max-w-full rounded-lg" src={process.env.NEXT_PUBLIC_STORAGE + item.image} alt="artefact" />
                                    </div>
                                ))
                              }
                            </div>
                      ...
                      <div className={`bg-white rounded-xl my-7`}>
                        ...
                        >
                          <div className={`pr-3 px-6`}>
                            ...
                            <iframe src={artefactData.map} width="750" height="450" allowfullscreen="true" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                    ...
                    >
                      <div className={`flex justify-between mb-5 items-center`}>
                        <h1 className={`font-bold text-xl`}>Artefak Lainnya</h1>
                        <Link href={`/artefacts`} className={`text-blue-500 text-sm`}>
                          See More
                        </Link>
                      </div>
                      <div className={`flex flex-col gap-5`}>
                        {shuffle.map((artefact, index) => (
                              ...
                              <Link href={'/artefacts/' + artefact.id}>
                                ...
                                <div className="bg-white rounded-bl-lg rounded-br-lg p-2">
                                  <h5 className={`text-gray-900 text-xl font-bold my-2`}>
                                    {
                                      artefact.name.length > 15 ? artefact.name.substring(0, 15) + '...' : artefact.name
                                    }
                                  </h5>
                                  <div className={`text-gray-500 text-sm`}
                                       dangerouslySetInnerHTML={{ __html: artefact.desc.substring(0, 100) + '...' }}/>
                                </div>
                              </Link>
                            </div>
