...
export default function page() {
  ...
  const [artefactDisplay, setArtefactDisplay] = useState([]);

  const handleSearch = (keyword) => {
    const filteredItems = artefacts.filter((data) =>
        data.name.toLowerCase().includes(keyword.toLowerCase())
    );
    setArtefactDisplay(filteredItems);
  };
  ...
  const fetchData = async () => {
    ...
    setArtefactDisplay(res.data.data);
  };
        ...
        <div className={`bg-gray-100 h-full`}>
          <div className={`container`}>
            ...
            <section className="mt-5">
              {artefactDisplay.length === 0 && <NotFound />}
              <div
                  className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7`}
              >
                {artefactDisplay.map((artefak, index) => (
                    <Link
                        key={index}
                        href={`/artefacts/${artefak.id}`}
                        className={`cursor-pointer hover:shadow-md transition-all`}
                    >
                      <img
                          src={
                              process.env.NEXT_PUBLIC_STORAGE + artefak.image_thumbnail
                          }
                          alt="artefacts"
                      ...
                      <div className="rounded-bl-lg rounded-br-lg p-2">
                        ...
                        >
                          {artefak.name.length > 20
                              ? artefak.name.substring(0, 20) + "..."
                              : artefak.name}
                        </Link>
                        <div
                            className={`text-gray-500 text-md`}
                            dangerouslySetInnerHTML={{
                              __html: artefak.desc.substring(0, 150) + "...",
                            }}
                        />
                      </div>
                    </Link>
