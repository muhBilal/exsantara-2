...
export default function page({params}) {
    ...
    const fetchData = async () => {
        ...
        const res = await axios.get(process.env.NEXT_PUBLIC_API + '/assignments');
        setAssignments(res.data.data);

        const res_data = await axios.get(process.env.NEXT_PUBLIC_API + '/assignments/' + params.id);
        const data = res_data.data.data;
        data.createdAt = new Date(data.created_at).toLocaleDateString('id-ID', options);
        setAssignmentsData(data)
    }

    const shuffleassignments = async (assignments) => {
        if (assignments) {
            const shuffled = [...assignments]; // Create a shallow copy of the assignments array
    ...
    return (
        <Layouts>
            {
                assignmentData?.length === 0 ? <NotFound /> : (
                            ...
                            <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
                                <div className={`w-full md:w-[80%] pr-0 md:pr-5`}>
                                    <div className={`bg-white rounded-xl `}>
                                        <img
                                            src={process.env.NEXT_PUBLIC_STORAGE + assignmentData.image}
                                            alt="assignment"
                                        ...
                                        >
                                            <div className={`w-full pr-3 px-6`}>
                                                <div className={`flex gap-5 items-center mb-3`}>
                                                    <span class="bg-blue-100 text-primary text-sm font-medium px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-blue-400 border border-blue-400">Soal</span>
                                                    <h1 className={`text-gray-400 text-sm`}>{assignmentData.createdAt}</h1>
                                                </div>
                                                ...
                                                <div className={`text-gray-500 text-lg`}
                                                        dangerouslySetInnerHTML={{__html: assignmentData.desc}}/>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className={`bg-white rounded-xl my-7`}>
                                        ...
                                        >
                                            <div className={`pr-3 px-6`}>
                                                ...
                                                <Link href={assignmentData.link_gform} className="text-blue-600" target="_blank">
                                                  {
                                                    assignmentData.link_gform
                                                  }
                                                </Link>
                                            </div>
                                        </div>
                                ...
                                >
                                    <div className={`flex justify-between mb-5 items-center`}>
                                        <h1 className={`font-bold text-xl`}>Soal Lainnya</h1>
                                        <Link href={`/assignments`} className={`text-blue-500 text-sm`}>
                                            See More
                                        </Link>
                                    </div>
                                    <div className={`flex flex-col gap-5`}>
                                        {shuffle.map((assignment, index) => (
                                            <div key={index} className="hover:shadow-md transition-all">
                                                <Link href={'/quiz/' + assignment.id}>
                                                    <img
                                                        src={`${process.env.NEXT_PUBLIC_STORAGE}${assignment.image}`}
                                                        alt="assignments"
                                                    ...
                                                    <div className="bg-white rounded-bl-lg rounded-br-lg p-2">
                                                        ...
                                                        {
                                                            assignment.name.length > 15 ? assignment.name.substring(0, 15) + '...' : assignment.name
                                                        }
                                                        </h5>
                                                        <div className={`text-gray-500 text-sm`}
                                                            dangerouslySetInnerHTML={{ __html: assignment.desc.substring(0, 100) + '...' }}/>
                                                    </div>
                                                </Link>
                                            </div>
