...
export default function page() {
  ...
  const handleSearch = (keyword) => {
    const filteredItems = quiz.filter((data) =>
        data.name.toLowerCase().includes(keyword.toLowerCase())
    );
    setQuizDisplay(filteredItems);
  };
        ...
        <div className={`bg-gray-100 h-full`}>
          <div className={`container`}>
            ...
            <section className="mt-5">
              {quizDisplay.length === 0 && <NotFound />}
              <div
                  className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7`}
              >
                {quizDisplay.map((quiz, index) => (
                    <Link
                        key={index}
                        href={`/quiz/${quiz.id}`}
                    ...
                    >
                      <img
                          src={
                              process.env.NEXT_PUBLIC_STORAGE + quiz.image_thumbnail
                          }
                          alt="quiz"
                      ...
                      <div className="rounded-bl-lg rounded-br-lg p-2">
                        ...
                        >
                          {quiz.name.length > 20
                              ? quiz.name.substring(0, 20) + "..."
                              : quiz.name}
                        </Link>
                        <div
                            className={`text-gray-500 text-md`}
                            dangerouslySetInnerHTML={{
                              __html: quiz.desc.substring(0, 150) + "...",
                            }}
                        />
                      </div>
                    </Link>
