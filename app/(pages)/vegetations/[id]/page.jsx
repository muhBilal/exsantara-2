...
export default function page({params}) {
    ...
    const shufflevegetation = async (vegetation) => {
        if (vegetation) {
            const shuffled = [...vegetation]; // Create a shallow copy of the vegetation array
    ...
    useEffect(() => {
        shufflevegetation(vegetation)
    }, [vegetation, vegetationData]);

    return (
        <Layouts>
            {
                vegetationData?.length === 0 ? <NotFound /> : (
                            ...
                            <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
                                        ...
                                        >
                                            <div className={`w-full pr-3 px-6`}>
                                                <div className={`flex gap-5 items-center mb-3`}>
                                                    <span class="bg-blue-100 text-primary text-sm font-medium px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-blue-400 border border-blue-400">Tumbuhan</span>
                                                    <h1 className={`text-gray-400 text-sm`}>{vegetationData.createdAt}</h1>
                                                </div>
                                        ...
                                        >
                                            <div className={`pr-3 px-6`}>
                                                ...
                                                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                                                    {
                                                        vegetationData.galleries?.map(item => (
                                                            <div>
                                    ...
                                    <div className={`bg-white rounded-xl my-7`}>
                                        ...
                                        >
                                            <div className={`pr-3 px-6`}>
                                                ...
                                                <iframe src={vegetationData.map} width="750" height="450" allowfullscreen="true" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                                ...
                                >
                                    <div className={`flex justify-between mb-5 items-center`}>
                                        <h1 className={`font-bold text-xl`}>Tumbuhan Lainnya</h1>
                                        <Link href={`/vegetation`} className={`text-blue-500 text-sm`}>
                                            See More
                                        </Link>
                                    </div>
                                    <div className={`flex flex-col gap-5`}>
                                        {shuffle.map((animal, index) => (
                                            <div key={index} className="hover:shadow-md transition-all">
                                                <Link href={'/vegetation/' + animal.id}>
                                                    <img
