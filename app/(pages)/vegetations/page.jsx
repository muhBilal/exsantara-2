      ...
      <div className={`bg-gray-100 h-full`}>
        <div className={`container`}>
            ...
            <form className="w-80">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Cari nama tumbuhan ..."
                  required
          ...
          <section className="mt-5">
              {vegetationsDisplay.length === 0 && (
                <NotFound />
              )}
            <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7`}>              
              {vegetationsDisplay.map((vegetation, index) => (
                <Link key={index}
                  href={`/vegetations/${vegetation.id}`}
                  className={`cursor-pointer hover:shadow-md transition-all`}
                >
                  <img
                    src={
                      process.env.NEXT_PUBLIC_STORAGE + vegetation.image_thumbnail
                    }
                    alt="vegetations"
                  ...
                  <div className="rounded-bl-lg rounded-br-lg p-2">
                    ...
                    >
                      {
                        vegetation.name.length > 20 ? vegetation.name.substring(0, 20) + '...' : vegetation.name
                      }
                    </Link>
                    <div
                      className={`text-gray-500 text-md`}
                      dangerouslySetInnerHTML={{ __html: vegetation.desc.substring(0, 150) + '...' }}
                    />
                  </div>
                </Link>
              ))}
