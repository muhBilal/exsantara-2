...
export default function page({params}) {
  ...
  const [customesData, setCustomesData] = useState([]);
  const [shuffle, setShuffle] = useState([]);

  const fetchData = async () => {
    const options = {year: 'numeric', month: 'long', day: 'numeric'}
    const res = await axios.get(process.env.NEXT_PUBLIC_API + '/cities/customes');
    setCustomes(res.data.data);

    const res_data = await axios.get(process.env.NEXT_PUBLIC_API + '/cities/customes/' + params.id);
    const data = res_data.data.data;
    data.createdAt = new Date(data.created_at).toLocaleDateString('id-ID', options);
    setCustomesData(data)
  }

  const shuffleData = async (customes) => {
    if (customes) {
      const shuffled = [...customes]; // Create a shallow copy of the customes array
  ...
  useEffect(() => {
    shuffleData(customes)
  }, [customes, customesData]);

  return (
      <Layouts>
        {
          customesData?.length === 0 ? <NotFound /> : (
                  ...
                  <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
                    <div className={`w-full md:w-[80%] pr-0 md:pr-5`}>
                      <div className={`bg-white rounded-xl `}>
                        <img
                            src={process.env.NEXT_PUBLIC_STORAGE + customesData.image_thumbnail}
                            alt="animal"
                        ...
                        >
                          <div className={`w-full pr-3 px-6`}>
                            <div className={`flex gap-5 items-center mb-3`}>
                              ...
                              <h1 className={`text-gray-400 text-sm`}>{customesData.createdAt}</h1>
                            </div>
                            ...
                            <div className={`text-gray-500 text-lg`}
                                 dangerouslySetInnerHTML={{__html: customesData.desc}}/>
                          </div>
                        </div>
                      </div>
                        ...
                        >
                          <div className={`pr-3 px-6`}>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                              {
                                customesData.galleries?.map(item => (
                                    <div>
                      ...
                      <div className={`bg-white rounded-xl my-7`}>
                        ...
                        >
                          <div className={`pr-3 px-6`}>
                            ...
                            <iframe src={customesData.map} width="750" height="450" allowfullscreen="true" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                    ...
                    >
                      <div className={`flex justify-between mb-5 items-center`}>
                        <h1 className={`font-bold text-xl`}>Pakaian Adat Lainnya</h1>
                      ...
                      <div className={`flex flex-col gap-5`}>
                        {shuffle.map((item, index) => (
                            <div key={index} className="hover:shadow-md transition-all">
                              <Link href={'/customes/' + item.id}>
                                <img
                                    src={`${process.env.NEXT_PUBLIC_STORAGE}${item.image_thumbnail}`}
                                    alt="animals"
