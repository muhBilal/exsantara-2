...
export default function page() {
  ...
  const [customesDisplay, setCustomesDisplay] = useState([]);

  const handleSearch = (keyword) => {
    const filteredItems = customes.filter((data) =>
        data.name.toLowerCase().includes(keyword.toLowerCase())
    );
    setCustomesDisplay(filteredItems);
  };
  ...
  const fetchData = async () => {
    ...
    setCustomesDisplay(res.data.data);
  };
        ...
        <div className={`bg-gray-100 h-full`}>
          <div className={`container`}>
            <section className={`flex justify-between items-center mb-10`}>
              <h1 className={`title`}>Pakaian Adat</h1>
              <form className="w-80">
                <label
            ...
            <section className="mt-5">
              {customesDisplay.length === 0 && <NotFound />}
              <div
                  className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7`}
              >
                {customesDisplay.map((custom, index) => (
                    <Link
                        key={index}
                        href={`/customes/${custom.id}`}
                        className={`cursor-pointer hover:shadow-md transition-all`}
                    >
                      <img
                          src={
                              process.env.NEXT_PUBLIC_STORAGE + custom.image_thumbnail
                          }
                          alt="customes"
                      ...
                      <div className="rounded-bl-lg rounded-br-lg p-2">
                        <Link
                            href={`/costumes/${custom.id}`}
                            className={`text-gray-900 text-2xl font-bold my-2 hover:text-blue-600 transition-all`}
                        >
                          {custom.name.length > 20
                              ? custom.name.substring(0, 20) + "..."
                              : custom.name}
                        </Link>
                        <div
                            className={`text-gray-500 text-md`}
                            dangerouslySetInnerHTML={{
                              __html: custom.desc.substring(0, 150) + "...",
                            }}
                        />
                      </div>
                    </Link>
