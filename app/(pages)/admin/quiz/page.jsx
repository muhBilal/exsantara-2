...
const page = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setAssignmentsDisplay(res.data.data);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.post(
          process.env.NEXT_PUBLIC_API + "/assignments/delete/" + id
        );
        fetchData();
  ...
  const handleSearch = (keyword) => {
    const filteredItems = assignments.filter((data) =>
      data.name.toLowerCase().includes(keyword.toLowerCase())
    );

    setAssignmentsDisplay(filteredItems);
  };
            ...
            <div className="flex-auto px-0 pt-0 pb-2">
              <div className="px-20 py-5 overflow-x-auto">
                <table className="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                  ...
                  <tbody>
                    {assignmentsDisplay.map((item, index) => (
                      <tr>
