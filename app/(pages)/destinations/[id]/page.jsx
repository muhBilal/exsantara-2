...
export default function page({params}) {
  ...
  const shuffleAnimals = async (destinations) => {
    if (destinations) {
      const shuffled = [...destinations]; // Create a shallow copy of the destinations array
  ...
  useEffect(() => {
    shuffleAnimals(destinations)
  }, [destinations, destinationData]);

  return (
      <Layouts>
        {
          destinationData?.length === 0 ? <NotFound /> : (
                  ...
                  <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
                        ...
                        >
                          <div className={`w-full pr-3 px-6`}>
                            <div className={`flex gap-5 items-center mb-3`}>
                              <span class="bg-blue-100 text-primary text-sm font-medium px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-blue-400 border border-blue-400">Destinasi</span>
                              <h1 className={`text-gray-400 text-sm`}>{destinationData.createdAt}</h1>
                            </div>
                        ...
                        >
                          <div className={`pr-3 px-6`}>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                              {
                                destinationData.galleries?.map(item => (
                                    <div>
                                      <img class="h-auto max-w-full rounded-lg" src={process.env.NEXT_PUBLIC_STORAGE + item.image} alt="destination" />
                                    </div>
                                ))
                              }
                            </div>
                      ...
                      <div className={`bg-white rounded-xl my-7`}>
                        ...
                        >
                          <div className={`pr-3 px-6`}>
                            ...
                            <iframe src={destinationData.map} width="750" height="450" allowfullscreen="true" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                    ...
                    >
                      <div className={`flex justify-between mb-5 items-center`}>
                        <h1 className={`font-bold text-xl`}>Destinasi Lainnya</h1>
                        <Link href={`/destinations`} className={`text-blue-500 text-sm`}>
                          See More
                        </Link>
                      </div>
                      <div className={`flex flex-col gap-5`}>
                        {shuffle.map((destination, index) => (
                              ...
                              <Link href={'/destinations/' + destination.id}>
                                ...
                                <div className="bg-white rounded-bl-lg rounded-br-lg p-2">
                                  <h5 className={`text-gray-900 text-xl font-bold my-2`}>
                                    {
                                      destination.name.length > 15 ? destination.name.substring(0, 15) + '...' : destination.name
                                    }
                                  </h5>
                                  <div className={`text-gray-500 text-sm`}
                                       dangerouslySetInnerHTML={{ __html: destination.desc.substring(0, 100) + '...' }}/>
                                </div>
                              </Link>
                            </div>
