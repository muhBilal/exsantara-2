...
export default function page() {
  ...
  const handleSearch = (keyword) => {
    const filteredItems = province.filter((data) =>
        data.name.toLowerCase().includes(keyword.toLowerCase())
    );
    setProvinceDisplay(filteredItems);
  };

  const fetchData = async () => {
    const res = await axios.get(
        process.env.NEXT_PUBLIC_API + "/provinces"
    );
    setProvince(res.data.data);
    setProvinceDisplay(res.data.data);
  };
        ...
        <div className={`bg-gray-100 h-full`}>
          <div className={`container`}>
            ...
            <section className="mt-5">
              {provinceDisplay.length === 0 && <NotFound />}
              <div
                  className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7`}
              >
                {provinceDisplay.map((animal, index) => (
                    <Link
                        key={index}
                      ...
                      <div className="rounded-bl-lg rounded-br-lg p-2">
                        ...
                        <div
                            ...
                            dangerouslySetInnerHTML={{
                              __html: animal.desc.substring(0, 150) + "...",
                            }}
                        />
                      </div>
                    </Link>
