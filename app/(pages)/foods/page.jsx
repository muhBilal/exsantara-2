      ...
      <div className={`bg-gray-100 h-full`}>
        <div className={`container`}>
          <section className={`flex justify-between items-center mb-10`}>
            <h1 className={`title`}>Makanan Daerah</h1>
            <form className="w-80">
              <label
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7`}>              
              {foodsDisplay.map((food, index) => (
                ...
                >
                  <img
                    src={
                      process.env.NEXT_PUBLIC_STORAGE + food.image_thumbnail
                    }
                    alt="foods"
                  ...
                  <div className="rounded-bl-lg rounded-br-lg p-2">
                    ...
                    >
                      {
                        food.name.length > 20 ? food.name.substring(0, 20) + '...' : food.name
                      }
                    </Link>
                    <div
                      className={`text-gray-500 text-md`}
                      dangerouslySetInnerHTML={{ __html: food.desc.substring(0, 150) + '...' }}
                    />
                  </div>
                </Link>
              ))}
