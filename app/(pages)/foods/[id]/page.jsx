...
export default function page({params}) {
    ...
    const fetchData = async () => {
        ...
        const res = await axios.get(process.env.NEXT_PUBLIC_API + '/provinces/foods');
        setFoods(res.data.data);

        const res_data = await axios.get(process.env.NEXT_PUBLIC_API + '/provinces/foods/' + params.id);
        const data = res_data.data.data;
        data.createdAt = new Date(data.created_at).toLocaleDateString('id-ID', options);
        setFoodData(data)
    }

    const shufflefoods = async (foods) => {
        if (foods) {
                            ...
                            <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
                                        ...
                                        >
                                            <div className={`w-full pr-3 px-6`}>
                                                <div className={`flex gap-5 items-center mb-3`}>
                                                    ...
                                                    <h1 className={`text-gray-400 text-sm`}>{foodData.createdAt}</h1>
                                                </div>
                                        ...
                                        >
                                            <div className={`pr-3 px-6`}>
                                                ...
                                                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                                                    {
                                                        foodData.galleries?.map(item => (
                                                            <div>
                                                                <img class="h-auto max-w-full rounded-lg" src={process.env.NEXT_PUBLIC_STORAGE + item.image} alt="food" />
                                                            </div>
                                                        ))
                                                    }
                                                </div>
                                    ...
                                    <div className={`bg-white rounded-xl my-7`}>
                                        ...
                                        >
                                            <div className={`pr-3 px-6`}>
                                                ...
                                                <iframe src={foodData.map} width="750" height="450" allowfullscreen="true" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                                ...
                                >
                                    <div className={`flex justify-between mb-5 items-center`}>
                                        <h1 className={`font-bold text-xl`}>Makanan Lainnya</h1>
                                        <Link href={`/foods`} className={`text-blue-500 text-sm`}>
                                            See More
                                        </Link>
                                    </div>
                                    <div className={`flex flex-col gap-5`}>
                                        {shuffle.map((food, index) => (
                                                ...
                                                <Link href={'/foods/' + food.id}>
                                                    ...
                                                    <div className="bg-white rounded-bl-lg rounded-br-lg p-2">
                                                        ...
                                                        <div className={`text-gray-500 text-sm`}
                                                            dangerouslySetInnerHTML={{ __html: food.desc.substring(0, 100) + '...' }}/>
                                                    </div>
                                                </Link>
                                            </div>
