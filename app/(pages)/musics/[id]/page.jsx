...
export default function page({params}) {
    ...
    const shufflemusics = async (musics) => {
        if (musics) {
    ...
    useEffect(() => {
        shufflemusics(musics)
    }, [musics, musicData]);

    return (
        <Layouts>
            {
                musicData?.length === 0 ? <NotFound /> : (
                            ...
                            <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
                                <div className={`w-full md:w-[80%] pr-0 md:pr-5`}>
                                    <div className={`bg-white rounded-xl `}>
                                        <img
                                            src={process.env.NEXT_PUBLIC_STORAGE + musicData.image}
                                            alt="music"
                                        ...
                                        >
                                            <div className={`w-full pr-3 px-6`}>
                                                <div className={`flex gap-5 items-center mb-3`}>
                                                    <span class="bg-blue-100 text-primary text-sm font-medium px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-blue-400 border border-blue-400">Musik</span>
                                                    <h1 className={`text-gray-400 text-sm`}>{musicData.createdAt}</h1>
                                                </div>
                                    ...
                                    <div className={`bg-white rounded-xl my-7`}>
                                        ...
                                        >
                                            <div className={`pr-3 px-6`}>
                                                ...
                                                <iframe width="750" height="450" src={musicData?.video} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                >
                                    <div className={`flex justify-between mb-5 items-center`}>
                                        <h1 className={`font-bold text-xl`}>Musik Lainnya</h1>
                                        <Link href={`/musics`} className={`text-blue-500 text-sm`}>
                                            See More
                                        </Link>
                                    </div>
                                    <div className={`flex flex-col gap-5`}>
                                        {shuffle.map((animal, index) => (
                                            <div key={index} className="hover:shadow-md transition-all">
                                                <Link href={'/musics/' + animal.id}>
                                                    <img
                                                        src={`${process.env.NEXT_PUBLIC_STORAGE}${animal.image}`}
                                                        alt="musics"
