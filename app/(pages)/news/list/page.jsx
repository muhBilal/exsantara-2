...
export default function page() {
  ...
  const [newssDisplay, setNewsDisplay] = useState([]);

  const handleSearch = (keyword) => {
    const filteredItems = news.filter((data) =>
        data.title.toLowerCase().includes(keyword.toLowerCase())
    );
    setNewsDisplay(filteredItems);
  };
        ...
        <div className={`bg-gray-100 h-full`}>
          <div className={`container`}>
            ...
            <section className="mt-5">
              {newssDisplay.length === 0 && <NotFound />}
              <div
                  className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7`}
              >
                {newssDisplay.map((news, index) => (
                    <Link
                        key={index}
                        href={`/news/${news.id}`}
                    ...
                    >
                      <img
                          src={
                              process.env.NEXT_PUBLIC_STORAGE + news.image
                          }
                          alt="news"
                      ...
                      <div className="rounded-bl-lg rounded-br-lg p-2">
                        ...
                        <div
                            ...
                            dangerouslySetInnerHTML={{
                              __html: news.desc.substring(0, 150) + "...",
                            }}
                        />
                      </div>
                    </Link>
